...
class DynamicStyles {
	...
	public function enqueue_scripts() {
		...
		wp_register_style( 'foodymat-dynamic', false, 'foodymat-main' );
		wp_enqueue_style( 'foodymat-dynamic' );
		wp_add_inline_style( 'foodymat-dynamic', $this->minify_css( $dynamic_css ) );
	}
	...
	private function inline_style() {
		...
		$border_color      	= foodymat_option( 'rt_border_color', '#e7e7e7' );
		$title_color     	= foodymat_option( 'rt_title_color', '#02060C' );;
		$button_color    	= foodymat_option( 'rt_button_color', '#ffffff' );
		$button_text_color 	= foodymat_option( 'rt_button_text_color', '#E9004B' );;
		$meta_color      	= foodymat_option( 'rt_meta_color', '#333333' );;
		$meta_light      	= foodymat_option( 'rt_meta_light', '#b2c2c0' );
		$gray10          	= foodymat_option( 'rt_gray10_color', '#f1f1f1' );
		$gray20          	= foodymat_option( 'rt_gray20_color', '#edf5f4' );
		...
		--rt-container-width: 	<?php echo foodymat_option( 'container_width' ); ?>px;
		}
	...
	protected function topbar_css() {
		$_topbar_active_color = foodymat_option( 'rt_topbar_active_color' );
		echo self::css( 'body .site-header .foodymat-topbar .topbar-container *:not(.dropdown-menu *)', 'color', 'rt_topbar_color' );
		echo self::css( 'body .site-header .foodymat-topbar .topbar-container svg:not(.dropdown-menu svg)', 'fill', 'rt_topbar_color' );

		if ( ! empty( $_topbar_active_color ) ) : ?>
			body .site-header .foodymat-topbar .topbar-container a:hover:not(.dropdown-menu a:hover),
			body .foodymat-topbar #topbar-menu ul ul li.current_page_item > a,
			body .foodymat-topbar #topbar-menu ul ul li.current-menu-ancestor > a,
			body .foodymat-topbar #topbar-menu ul.foodymat-topbar-menu > li.current-menu-item > a,
			body .foodymat-topbar #topbar-menu ul.foodymat-topbar-menu > li.current-menu-ancestor > a {
			color: <?php echo esc_attr( $_topbar_active_color ); ?>;
			}

			body .site-header .foodymat-topbar .topbar-container a:hover:not(.dropdown-menu a:hover svg) svg,
			body .foodymat-topbar #topbar-menu ul ul li.current-menu-ancestor > a svg,
			body .foodymat-topbar #topbar-menu ul.foodymat-topbar-menu > li.current-menu-item > a svg,
			body .foodymat-topbar #topbar-menu ul.foodymat-topbar-menu > li.current-menu-ancestor > a svg {
			fill: <?php echo esc_attr( $_topbar_active_color ); ?>;
			}
		<?php endif; ?>

		<?php
		echo self::css( 'body .foodymat-topbar', 'background-color', 'rt_topbar_bg_color' );
	...
	protected function header_css() {
		...
		$logo_dimension     = foodymat_option( 'rt_logo_width_height' );
		$logo_mobile_dimension     = foodymat_option( 'rt_mobile_logo_width_height' );
		$menu_border_bottom = foodymat_option( 'rt_menu_border_color' );
		...
		//Default Menu
		$_menu_color        = foodymat_option( 'rt_menu_color' );
		$_menu_active_color = foodymat_option( 'rt_menu_active_color' );
		$_menu_bg_color     = foodymat_option( 'rt_menu_bg_color' );
		$_sub_menu_bg_color     = foodymat_option( 'rt_sub_menu_bg_color' );

		//Transparent Menu
		$_tr_menu_color        = foodymat_option( 'rt_tr_menu_color' );
		$_tr_menu_active_color = foodymat_option( 'rt_tr_menu_active_color' );

		$_header_border     = foodymat_option( 'rt_header_border' );
		$_breadcrumb_border = foodymat_option( 'rt_breadcrumb_border' );
		$_preloader_bg_color = foodymat_option( 'preloader_bg_color' );
		?>

		<?php //Header Logo CSS ?>
		<?php if ( Opt::$header_width == 'full' ) :
			$h_width = '100%';
			if ( ( $header_width = foodymat_option( 'rt_header_max_width' ) ) > 768 ) {
		...
		<?php if ( ! empty( $_menu_color ) ) : ?>
			body .foodymat-navigation ul li a,
			body .foodymat-offcanvas-drawer ul.menu li a,
			body .foodymat-navigation ul li ul li a,
			body .menu-icon-wrapper .menu-search-bar {
				color: <?php echo esc_attr( $_menu_color ) ?>;
			}
			body .main-header-section svg,
			body .foodymat-navigation .caret svg {
		...
		<?php if ( ! empty( $_menu_active_color ) ) : ?>
			body .foodymat-navigation ul li a:hover,
			body .foodymat-navigation ul li.current-menu-item > a,
			body .foodymat-navigation ul li.current-menu-ancestor > a,
			body .foodymat-offcanvas-drawer ul li.current-menu-ancestor > a,
			body .foodymat-offcanvas-drawer ul.menu li a:hover,
			body .foodymat-navigation ul li ul li a:hover {
				color: <?php echo esc_attr( $_menu_active_color ) ?>;
			}
			body .foodymat-navigation ul li a:hover svg,
			body .foodymat-navigation ul li.current-menu-item > a svg,
			body .foodymat-navigation ul li.current-menu-ancestor > a svg {
		...
		<?php if ( ! empty( $_sub_menu_bg_color ) ) : ?>
			body .foodymat-navigation ul > li > ul,
			body .foodymat-navigation ul li.mega-menu > ul.dropdown-menu{
		...
		<?php if ( ! empty( $_tr_menu_color ) ) : ?>
			...
			body.has-trheader .site-header .foodymat-navigation *,
			body.has-trheader .site-header .foodymat-navigation ul li a {
			...
			body.has-trheader .site-header .foodymat-topbar .caret svg,
		...
		<?php if ( ! empty( $_tr_menu_active_color ) ) : ?>
			body.has-trheader .site-header .foodymat-navigation ul li a:hover,
			body.has-trheader .site-header .foodymat-navigation ul li.current-menu-item > a,
			body.has-trheader .site-header .foodymat-navigation ul li.current-menu-ancestor > a {
			...
			body.has-trheader .site-header .foodymat-navigation ul li.current-menu-ancestor > a svg,
			body.has-trheader .site-header .foodymat-navigation ul li.current-menu-item > a svg {
			fill: <?php echo esc_attr( $_tr_menu_active_color ); ?>
			}
		<?php endif; ?>
		<?php if ( ! empty( $menu_border_bottom ) ) : ?>
			body .foodymat-topbar,
			body .main-header-section,
			body .foodymat-breadcrumb-wrapper {
		...
		<?php if ( ! $_breadcrumb_border ) : ?>
			body .foodymat-breadcrumb-wrapper {border-bottom: none;}
		<?php endif; ?>
	...
	protected function breadcrumb_css() {
		$breadcrumb_color          = foodymat_option( 'rt_breadcrumb_color' );
		$rt_breadcrumb_hover       = foodymat_option( 'rt_breadcrumb_hover' );
		$breadcrumb_active         = foodymat_option( 'rt_breadcrumb_active' );
		$rt_breadcrumb_title_color = foodymat_option( 'rt_breadcrumb_title_color' );
		$rt_banner_color_opacity = foodymat_option( 'rt_banner_color_opacity' );

		$rt_banner_padding_top = foodymat_option( 'rt_banner_padding_top' );
		$rt_banner_padding_bottom = foodymat_option( 'rt_banner_padding_bottom' );

		$rt_banner_overlay1_color = foodymat_option( 'rt_banner_overlay1_color' );
		$rt_banner_overlay2_color = foodymat_option( 'rt_banner_overlay2_color' );
		...
		if ( ! empty( $rt_banner_color_opacity ) ) { ?>
			.foodymat-breadcrumb-wrapper.has-bg .banner-image {
		...
		if ( ! empty( $rt_banner_overlay1_color && $rt_banner_overlay2_color ) ) { ?>
			.foodymat-breadcrumb-wrapper.has-bg::before {
	...
	protected function content_padding_css() {
		...
		protected function footer_css() {
			if ( foodymat_option( 'rt_footer_width' ) && foodymat_option( 'rt_footer_max_width' ) > 1400 ) {
			...
			echo self::css( 'body .site-footer .widget-title, .foodymat-footer-2 .site-footer .widget-title', 'color', 'rt_footer_widget_title_color' );
		...
		function site_fonts() {
			...
			$typo_body           = json_decode( foodymat_option( 'rt_body_typo' ), true );
			$typo_menu           = json_decode( foodymat_option( 'rt_menu_typo' ), true );
			$typo_heading        = json_decode( foodymat_option( 'rt_all_heading_typo' ), true );
			$body_font_family    = $typo_body['font'] ?? 'Quicksand';
			...
			echo self::font_css( '.foodymat-navigation ul li a', [
				'size'          => $typo_menu['size'],
			...
			foreach ( $heading_fonts as $heading ) {
				$font = json_decode( foodymat_option( "rt_heading_{$heading}_typo" ), true );
				if ( ! empty( $font['font'] ) ) {
		...
		static function css( $selector, $property, $theme_mod, $after_css = '' ) {
			$theme_mod = foodymat_option( $theme_mod );
