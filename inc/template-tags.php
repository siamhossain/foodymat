...
use RT\Foodymat\Modules\PostShare;

/*Allow HTML for the kses post*/
function foodymat_html( $html, $context = '' ) {
...
if ( ! function_exists( 'foodymat_custom_menu_cb' ) ) {
	/**
	 ...
	 * @return string|void
	 */
	function foodymat_custom_menu_cb( $args ) {
		extract( $args );
		$add_menu_link = admin_url( 'nav-menus.php' );
		$menu_text     = sprintf( __( "Add %s Menu", "foodymat" ), $theme_location );
		__( 'Add a menu', 'foodymat' );
		if ( ! current_user_can( 'manage_options' ) ) {
			$add_menu_link = home_url();
			$menu_text     = __( 'Home', 'foodymat' );
		}
...
if ( ! function_exists( 'foodymat_menu_icons_group' ) ) {
	/**
	 * Get menu icon group
	 * @return void
	 */
	function foodymat_menu_icons_group( $args = [] ) {
		$default_args = [
			'hamburg'       => foodymat_option( 'rt_header_bar' ),
			'search'        => foodymat_option( 'rt_header_search' ),
			'login'         => foodymat_option( 'rt_header_login_link' ),
			'button'        => foodymat_option( 'rt_get_started_button' ),
			'button_label'  => foodymat_option( 'rt_get_started_label' ),
			'button_link'   => foodymat_option( 'rt_get_started_button_url' ),
			'has_separator' => foodymat_option( 'rt_header_separator' ),
			'cart' => foodymat_option( 'rt_header_cart' ),
		];
			...
			<ul class="menu-icon-action <?php echo esc_attr( $menu_classes ) ?>">
				...
				<?php if ( $args['search'] ) : ?>
					<li class="rt-search-popup">
						<a class="menu-search-bar rt-search-trigger menu-icon" href="#header-search" aria-label="search popup"><i class="icon-rt-search"></i></a>
					</li>
				<?php endif; ?>
				...
				<?php if ( $args['login'] ) : ?>
					<li class="rt-user-login">
						<a  class="menu-icon" href="<?php echo esc_url( wp_login_url() ) ?>" aria-label="user login">
...
if ( ! function_exists( 'foodymat_require' ) ) {
	/**
	 ...
	 * @return false|void
	 */
	function foodymat_require( $filename, string $dir = 'inc' ) {
...
if ( ! function_exists( 'foodymat_readmore_text' ) ) {
	/**
	 * Creates continue reading text.
	 *
	 * @return string
	 */
	function foodymat_readmore_text() {

		if ( empty( foodymat_option( 'rt_blog_read_more' ) ) ) {
			return;
		}
		return sprintf(
			'%s %s',
			esc_html( foodymat_option( 'rt_blog_read_more' ) ),
			the_title( '<span class="screen-reader-text">', '</span>', false )
		);
	}
}

if ( ! function_exists( 'foodymat_list_item_separator' ) ) {
	/**
	 ...
	 * @since 6.0.0
	 */
	function foodymat_list_item_separator() {
...
if ( ! function_exists( 'foodymat_posted_in' ) ) {
	/**
	 * Prints HTML with category list information about theme categories.
	 * @return string
	 */
	function foodymat_posted_in( $type = 'category' ) {
		$categories_list = get_the_category_list( foodymat_list_item_separator() );
		if ( 'tag' === $type ) {
			$categories_list = get_the_tag_list( '', foodymat_list_item_separator() );
		}
		if ( $categories_list ) {
			return sprintf(
...
if ( ! function_exists( 'foodymat_posted_on' ) ) {
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 * @return string
	 */
	function foodymat_posted_on() {
		$time_string = sprintf(
...
if ( ! function_exists( 'foodymat_posted_by' ) ) {
	/**
	 * Prints HTML with meta information about theme author.
	 * @return string
	 */
	function foodymat_posted_by( $prefix = '' ) {
		return sprintf(
			esc_html__( '%s %s', 'foodymat' ),
...
if ( ! function_exists( 'foodymat_get_svg' ) ) {
	/**
	 * Get svg icon
	 *
	 * @param $name
	 ...
	 * @return string|void
	 */
	function foodymat_get_svg( $name, $rotate = '' ) {
		$svg_list     = apply_filters( 'foodymat_svg_icon_list', [
			...
			'share-stroke' 	   => '<svg width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M13.5363 13.0625C13.4529 13.0637 13.3707 13.0426 13.2982 13.0014C13.2257 12.9602 13.1655 12.9004 13.1238 12.8281C12.4995 11.7646 11.6097 10.8814 10.5415 10.2652C9.47324 9.64887 8.26328 9.32061 7.03006 9.3125V12.125C7.0296 12.2175 7.00175 12.3079 6.95004 12.3846C6.89832 12.4614 6.82504 12.5211 6.73944 12.5563C6.65407 12.5922 6.55999 12.602 6.46906 12.5844C6.37812 12.5669 6.29442 12.5229 6.2285 12.4578L0.603499 6.83282C0.559564 6.78924 0.524692 6.7374 0.500894 6.68027C0.477096 6.62315 0.464844 6.56188 0.464844 6.5C0.464844 6.43812 0.477096 6.37686 0.500894 6.31973C0.524692 6.26261 0.559564 6.21077 0.603499 6.16719L6.2285 0.542192C6.29442 0.477149 6.37812 0.433088 6.46906 0.415569C6.55999 0.39805 6.65407 0.407858 6.73944 0.443754C6.82504 0.47892 6.89832 0.538638 6.95004 0.615382C7.00175 0.692125 7.0296 0.782461 7.03006 0.875004V3.73907C8.96937 3.98816 10.752 4.93389 12.0456 6.4C13.3392 7.86611 14.0556 9.75258 14.0613 11.7078C14.0602 12.0213 14.0399 12.3344 14.0004 12.6453C13.9887 12.7428 13.9466 12.8342 13.8802 12.9065C13.8137 12.9789 13.7262 13.0285 13.6301 13.0484L13.5363 13.0625ZM6.79569 8.375C7.99119 8.35783 9.17617 8.60042 10.2687 9.08601C11.3613 9.5716 12.3354 10.2886 13.1238 11.1875C12.9828 9.48706 12.2405 7.89249 11.0301 6.68984C9.81973 5.48718 8.22044 4.75513 6.51912 4.625C6.40225 4.61444 6.2936 4.56043 6.21462 4.47364C6.13564 4.38685 6.09209 4.2736 6.09256 4.15625V2.00469L1.59725 6.5L6.09256 10.9953V8.84375C6.09256 8.71943 6.14195 8.6002 6.22986 8.5123C6.31776 8.42439 6.43699 8.375 6.56131 8.375H6.81444H6.79569Z" fill="#8D8D8D"/></svg>
',
...
if ( ! function_exists( 'foodymat_get_file' ) ) {
	/**
	 * Get File Path
	 *
	 * @param $path
	 ...
	 * @return string
	 */
	function foodymat_get_file( $path, $return_path = false ): string {
...
if ( ! function_exists( 'foodymat_get_img' ) ) {
	/**
	 * Get Image Path
	 *
	 * @param $filename
	 ...
	 * @return string|void
	 */
	function foodymat_get_img( $filename, $echo = false, $image_meta = '' ) {
		$path      = '/assets/images/' . $filename;
		$image_url = foodymat_get_file( $path );

		if ( $echo ) {
			if ( ! strpos( $filename, '.svg' ) ) {
				$getimagesize = wp_getimagesize( foodymat_get_file( $path, true ) );
...
if ( ! function_exists( 'foodymat_get_css' ) ) {
	/**
	 * Get CSS Path
	 *
	 * @param $filename
	 ...
	 * @return string
	 */
	function foodymat_get_css( $filename, $check_rtl = false ) {
		$min    = WP_DEBUG ? '.css' : '.css';
		$is_rtl = $check_rtl && is_rtl() ? 'css-rtl' : 'css';
		$path   = "/assets/$is_rtl/" . $filename . $min;

		return foodymat_get_file( $path );
	}
}

if ( ! function_exists( 'foodymat_get_js' ) ) {
	/**
	 * Get JS Path
	 *
	 * @param $filename
	 ...
	 * @return string
	 */
	function foodymat_get_js( $filename ) {
...
if ( ! function_exists( 'foodymat_option' ) ) {
	/**
	 ...
	 * @return mixed
	 */
	function foodymat_option( $key, $default = '', $return_array = false ) {
...
if ( ! function_exists( 'foodymat_get_social_html' ) ) {
	/**
	 * Get Social markup
	 *
	 * @param $color
	...
	function foodymat_get_social_html( $color = '' ) {
		ob_start();
...
if ( ! function_exists( 'foodymat_site_logo' ) ) {
	/**
	 * Newfit Site Logo
	 *
	 */
	function foodymat_site_logo( $with_logo = false, $custom_title = '', $mode = '' ) {
		$logo_main       = foodymat_option( 'rt_logo' );
		$logo_light      = foodymat_option( 'rt_logo_light' );
		$logo_mobile     = foodymat_option( 'rt_logo_mobile' );
...
if ( ! function_exists( 'foodymat_footer_logo' ) ) {
	/**
	 * Newfit Site Logo
	 *
	 */
	function foodymat_footer_logo() {
		$main_logo  = foodymat_option( 'rt_logo' );
		$logo_light = foodymat_option( 'rt_logo_light' );
		$site_logo  = $main_logo;
...
if ( ! function_exists( 'foodymat_scroll_top' ) ) {
	/**
	 * Back-to-top button
	 * @return void
	 */
	function foodymat_scroll_top( $class = '', $icon = 'scroll-top' ) {
		if ( foodymat_option( 'rt_back_to_top' ) ) {
			?>
...
if ( ! function_exists( 'foodymat_meta_icons' ) ) {
	/**
	 * Get meta icons
	 *
	 * @param $name
	 ...
	 * @return string|void
	 */
	function foodymat_meta_icons( $name ) {
		if ( ! $name ) {
			return;
		}
		$icon_list = [
			'author'   => '<i class="icon-rt-user"></i>',
			'date'     => '<i class="icon-caleder-f"></i>',
...
if ( ! function_exists( 'foodymat_reading_time' ) ) {
	/**
	 * Post reading time
	 */
	function foodymat_reading_time() {
		...
		if ( $reading_time < 1 ) {
			$result = esc_html__( 'Less than a minute', 'foodymat' );
		} elseif ( $reading_time > 60 ) {
			$result = sprintf( esc_html__( '%s hours read', 'foodymat' ), floor( $reading_time / 60 ) );
		} else if ( $reading_time == 1 ) {
			$result = esc_html__( '1 min read', 'foodymat' );
		} else {
			$result = sprintf( esc_html__( '%s mins read', 'foodymat' ), $reading_time );
		}
...
if( !function_exists( 'rt_post_views' )){
	...
	function rt_post_views( $text = '', $post_id = 0 ){
		...
		if ( $view_count == 1 ) {
			$foodymat_view_html = esc_html__( 'View' , 'foodymat' );
		} else {
			$foodymat_view_html = esc_html__( 'Views' , 'foodymat' );
		}

		$foodymat_views_html = '<span class="view-number" >' . $view_count . '</span> ' . $foodymat_view_html;

		return '<span class="meta-views meta-item '. $views_class .'">'. $foodymat_views_html.'</span> ';
	}
}

if ( ! function_exists( 'foodymat_post_meta' ) ) {
	/**
	 * Get post meta
	 *
	 * @return string
	 */
	function foodymat_post_meta( $args ) {
		$default_args = [
			'with_list'     => true,
			'with_icon'     => true,
			'include'       => [],
			'class'         => '',
			'author_prefix' => __( 'By', 'foodymat' )
		];

		$args = wp_parse_args( $args, $default_args );

		$comments_number = get_comments_number();
		$comments_text   = sprintf( _n( 'Comment: %s', 'Comments: %s', $comments_number, 'foodymat' ), number_format_i18n( $comments_number ) );

		$_meta_data = [];
		$output     = '';

		$_meta_data['author']   = foodymat_posted_by( $args['author_prefix'] );
		$_meta_data['category'] = foodymat_posted_in();
		$_meta_data['date']     = foodymat_posted_on();
		$_meta_data['tag']      = foodymat_posted_in( 'tag' );
		$_meta_data['comment']  = esc_html( $comments_text );
		$_meta_data['reading']  = foodymat_reading_time();
		...
		foreach ( $meta_list as $key ) {
			...
			$output .= $args['with_icon'] ? foodymat_meta_icons( $key ) : null;
			$output .= $meta;
...
if ( ! function_exists( 'foodymat_post_thumbnail' ) ) {
	/**
	 * Displays post thumbnail.
	 * @return void
	 */
	function foodymat_post_thumbnail( $size = 'full', $thumb_date = false ) {
...
if ( ! function_exists( 'foodymat_post_single_thumbnail' ) ) {
	/**
	 * Display post details thumbnail
	 * @return void
	 */
	function foodymat_post_single_thumbnail( $size = 'full' ) {
		if ( ! Fns::can_show_post_thumbnail() ) {
			return;
		}
		?>
		...
		<?php } else { ?>
			...
			<?php if ( wp_get_attachment_caption( get_post_thumbnail_id() ) && foodymat_option( 'rt_single_caption_visibility' ) == 1 ) : ?>
				<figcaption class="wp-caption-text">
					<span><?php foodymat_html( wp_get_attachment_caption( get_post_thumbnail_id() ) ); ?></span>
				</figcaption>
			<?php endif; ?>
		<?php } ?>
		</div>
...
if ( ! function_exists( 'foodymat_entry_footer' ) ) {
	/**
	 * Foodymat Entry Footer
	 *
	 * @return void
	 *
	 */
		...
		if ( ! is_single() ) {
			...
			if ( ( has_tag() && foodymat_option( 'rt_single_tag_visibility' ) ) || foodymat_option( 'rt_single_share_visibility' ) ) { ?>
				<footer class="entry-footer d-flex align-items-center justify-content-between">
					<?php if ( foodymat_option( 'rt_single_tag_visibility' ) ) { ?>
						<div class="post-tags">
							<?php if ( $tags_label = foodymat_option( 'rt_tags' ) ) {
								printf( "<span>%s</span>", esc_html( $tags_label ) );
							} ?>

							<?php
							foodymat_single_post_footer_meta(
								'content-below-meta', [ 'tag' ]
							);
							?>
						</div>
					<?php }
					if ( foodymat_option( 'rt_single_share_visibility' ) ) { ?>
						<div class="post-share">
							<?php if ( $tags_label = foodymat_option( 'rt_social' ) ) {
								printf( "<span>%s</span>", esc_html( $tags_label ) );
							} ?>
							<?php PostShare::foodymat_post_share(); ?>
						</div>
					<?php } ?>
				</footer>
				<?php
...
// single video
if ( ! function_exists( 'foodymat_post_single_video' ) ) {
	/**
	 * Display post details video
	 * @return void
	 */
	function foodymat_post_single_video() { ?>
		<?php $rt_youtube_link = get_post_meta( get_the_ID(), 'rt_youtube_link', true );
...
if ( ! function_exists( 'foodymat_entry_profile' ) ) {
	/**
	 * Foodymat Entry Profile
	 *
	 * @return void
	 *
	 */
	function foodymat_entry_profile() {
		if ( foodymat_option( 'rt_single_profile_visibility' ) ) {
...
if ( ! function_exists( 'foodymat_single_post_footer_meta' ) ) {
	/**
	 * Get single post footer meta
	 *
	 * @return string
	 */
	function foodymat_single_post_footer_meta( $class = '', $includes = [ 'tag' ] ) {
		if ( is_single() && foodymat_option( 'rt_single_tag_visibility' ) ) : ?>
			<div class="post-footer-meta <?php echo esc_attr( $class ) ?>">
				<?php echo foodymat_post_meta( [
					'with_list' => false,
					'with_icon' => false,
					'include'   => $includes,
				] ); ?>
...
if ( ! function_exists( 'foodymat_entry_content' ) ) {
	/**
	 * Entry Content
	 * @return void
	 */
	function foodymat_entry_content() {
		if ( ! is_single() ) {
			$length = foodymat_option( 'rt_excerpt_limit' );
...
if ( ! function_exists( 'foodymat_sidebar' ) ) {
	/**
	 ...
	 * @return false|void
	 */
	function foodymat_sidebar( $sidebar_id ) {
...
if ( ! function_exists( 'foodymat_post_class' ) ) {
	/**
	 * Get dynamic article classes
	 * @return string
	 */
	function foodymat_post_class( $default_class = 'foodymat-post-card rt-grid-item' ) {
		$above_meta_style = 'above-' . foodymat_option( 'rt_single_above_meta_style' );


		$common_clsss = '';
		if ( is_single() ) {
			$common_clsss .= foodymat_option( 'rt_single_above_meta_visibility' ) ? 'is-above-meta' : 'no-above-meta';
			$meta_style   = foodymat_option( 'rt_single_meta_style' );
			$post_classes = Fns::class_list( [ $common_clsss, $meta_style, $above_meta_style ] );
		} else {
			$common_clsss .= foodymat_option( 'rt_blog_above_meta_visibility' ) ? 'is-above-meta' : 'no-above-meta';
			$meta_style   = foodymat_option( 'rt_blog_meta_style' );
			$blog_style   = 'blog-' . foodymat_option( 'rt_blog_style' );
...
if ( ! function_exists( 'foodymat_separate_meta' ) ) {
	/**
	 * Get above title meta
	 * @return string
	 */
	function foodymat_separate_meta( $class = '', $includes = [ 'category' ] ) {
		if ( ( ! is_single() && foodymat_option( 'rt_blog_above_meta_visibility' ) ) || ( is_single() && foodymat_option( 'rt_single_above_meta_visibility' ) ) ) : ?>
...
if ( ! function_exists( 'foodymat_single_entry_header' ) ) {
	/**
	 * Get above title meta
	 * @return string
	 */
	function foodymat_single_entry_header() {
		?>
		<header class="entry-header">
			...
			if ( ! empty( Fns::single_meta_lists() ) && foodymat_option( 'rt_single_meta_visibility' ) ) {
...
if ( ! function_exists( 'foodymat_breadcrumb' ) ) {
	/**
	 * Foodymat breadcrumb
	 * @return void
	 */
	function foodymat_breadcrumb() {
		?>
		<nav aria-label="breadcrumb">
			<ul class="breadcrumb">
				<li class="breadcrumb-item">
					<a href="<?php echo esc_url( home_url() ); ?>"><?php esc_html_e( 'Home', 'foodymat' ) ?></a>
					<span class="raquo"><i class="icon-rt-user-datalist-feature"></i></span>
				</li>
				<li class="breadcrumb-item active" aria-current="page">
					<?php
					if ( is_tag() ) {
						esc_html_e( 'Posts Tagged: ', 'foodymat' );
						?>
						...
						esc_html_e( 'Posts made in: ', 'foodymat' );
						echo esc_html( get_the_time( is_year() ? 'Y' : ( is_month() ? 'F, Y' : 'F jS, Y' ) ) );
						echo '</span>';
					} elseif ( is_search() ) {
						echo '<span class="title">';
						esc_html_e( 'Search results for: ', 'foodymat' );
						the_search_query();
						echo '</span>';
					} elseif ( is_404() ) {
						echo '<span class="title">';
						esc_html_e( '404', 'foodymat' );
						echo '</span>';
					} elseif ( is_single() ) {
						...
						esc_html_e( 'Posts Category: ', 'foodymat' );
						echo '<span class="title">';
						single_cat_title();
						echo '</span>';
					} elseif ( is_tax() ) {
						...
						echo '<span class="title">';
						esc_html_e( 'Blog', 'foodymat' );
						echo '</span>';
					} elseif ( class_exists( 'WooCommerce' ) and is_shop() ) {
						echo '<span class="title">';
						esc_html_e( 'Shop', 'foodymat' );
						echo '</span>';
					}
					?>
				</li>
...
if ( ! function_exists( 'foodymat_get_avatar_url' ) ) :
	function foodymat_get_avatar_url( $get_avatar ) {
...
function foodymat_comments_cbf( $comment, $args, $depth ) {
	...
	switch ( $comment->comment_type ) :
		...
		case 'trackback' : ?>
			<div class="pingback-entry"><span
					class="pingback-heading"><?php esc_html_e( 'Pingback:', 'foodymat' ); ?></span> <?php comment_author_link(); ?>
			</div>
			<?php
			break;
		default :
				...
				<div class="author-info">
					...
					<div class="comment-details">
						...
						if ( $comment->comment_approved == '0' ) { ?>
							<em class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'foodymat' ); ?></em>
							<br/><?php
						} ?>

						<?php
						$icon = foodymat_get_svg( 'share-stroke' );
						// Display comment reply link
						comment_reply_link( array_merge( $args, [
							'add_below'  => $add_below,
							'depth'      => $depth,
							'max_depth'  => $args['max_depth'],
							'reply_text' => $icon . __( '', 'foodymat' )
						] ) ); ?>
...
if ( ! function_exists( 'foodymat_hanburger' ) ) {
	 ...
	 * @return void
	 */
	function foodymat_hanburger( $class = '' ) {
		?>
		<li class="ham-burger <?php echo esc_attr( $class ) ?>">
			<button type="button" class="menu-bar trigger-off-canvas" aria-label="hamburger menu">
				<?php if( foodymat_option('rt_get_menu_label') ) { ?>
					<span class="menu-label"><?php echo esc_html( foodymat_option('rt_get_menu_label') ) ?></span>
				<?php } ?>
