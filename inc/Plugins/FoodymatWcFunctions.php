...
class FoodymatWcFunctions {
	...
	public function __construct() {
		...
		add_action('woocommerce_share',[__CLASS__,'foodymat_product_share']);
	...
	public function loop_shop_per_page(){
		$shop_posts_per_page = foodymat_option('products_per_page');
	...
	public function loop_shop_columns(  ) {
		if( isset($_GET["displayview"]) && $_GET["displayview"] == 'list' ) {
			...
			$cols = foodymat_option('products_cols_width');
		}
		return $cols;
	}
	//Shop category
	...
	public function sale_flash( $args, $post, $product  ) {
		if ( 'percentage' == foodymat_option('wc_sale_label')  ) {
			...
			return sprintf('<span class="text onsale rt-prodcut-badge-1">' . esc_html__( 'Sale!', 'foodymat' ) . '</span>', $discount );
		}
	}
	...
	public static function print_add_to_cart_icon( $product_id, $icon = true, $text = true ){
		if ( defined( 'ELEMENTOR_PATH' ) && class_exists( 'Elementor\Widget_Base' ) ) {
			if ( \Elementor\Plugin::$instance->preview->is_preview_mode() || \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
				echo '<i class="icon-star-circle"></i>';
				return;
			}
		}
	...
	public function flash_sale_countdown_positions( $positions ) {
		...
		$positions['before_add_to_cart']['hooks']= [
			'foodymat_shop_layout_before_cart_button'
		];
		$positions['after_add_to_cart']['hooks']= [
			'foodymat_shop_layout_after_cart_button'
		];

		return $positions;
	}

	public static function foodymat_product_share(  ) {
		echo '<div class="product-single-social-shares-btns">';
		self::foodymat_product_social_share();
		echo '</div>';
	}

	public static function foodymat_product_social_share(){
		...
		<div class="post-share-btn">
			<ul class="item-social">
				<li class="item-label"><?php esc_html_e( 'Share:', 'foodymat' );?></li>
