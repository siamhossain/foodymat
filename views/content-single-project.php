		...
		<div class="project-item-wrap">
			<div class="project-content-info sidebar-sticky">
				<div class="project-information">
					<?php if ( !empty( $rt_project_title ) && foodymat_option( 'rt_project_title' )) { ?>
					...
					<?php } if ( !empty( $rt_project_text ) && foodymat_option( 'rt_project_text' ) ) { ?>
						<p><?php echo esc_html( $rt_project_text );?></p>
					<?php } ?>
					<ul class="info-list">
						<?php if ( foodymat_option( 'rt_project_cat' ) ) { ?>
							<li><label><?php esc_html_e( 'Category', 'foodymat' );?>: </label>
						...
						<?php } ?>
						<?php if ( !empty( $rt_project_client ) && foodymat_option( 'rt_project_client' ) ) { ?>
							<li><label><?php esc_html_e( 'Client', 'foodymat' );?>: </label><?php echo esc_html( $rt_project_client );?></li>
						<?php } if ( !empty( $rt_project_start ) && foodymat_option( 'rt_project_start' ) ) { ?>
							<li><label><?php esc_html_e( 'Starts On', 'foodymat' );?>: </label><?php echo esc_html( $rt_project_start );?></li>
						<?php } if ( !empty( $rt_project_end ) && foodymat_option( 'rt_project_end' ) ) { ?>
							<li><label><?php esc_html_e( 'Ends On', 'foodymat' );?>: </label><?php echo esc_html( $rt_project_end );?></li>
						<?php } if ( !empty( $rt_project_weblink ) && foodymat_option( 'rt_project_weblink' ) ) { ?>
							<li><label><?php esc_html_e( 'Web Link', 'foodymat' );?>: </label><?php echo esc_html( $rt_project_weblink );?></li>
						<?php } ?>

						<?php if( foodymat_option( 'rt_project_rating' ) ) { ?>
							<?php if( $ratting != -1) { ?>
								<li><label><?php esc_html_e( 'Rating', 'foodymat' );?>: </label>
									<ul class="rating">
